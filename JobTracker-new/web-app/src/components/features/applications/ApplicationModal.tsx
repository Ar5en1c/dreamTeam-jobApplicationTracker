...
const statusOptions: { value: ApplicationStatus; label: string; color: string }[] = [
  ...
  { value: 'final_interview', label: 'Final Interview', color: 'bg-indigo-100 text-indigo-800' },
...
export const ApplicationModal: React.FC<ApplicationModalProps> = ({
  ...
  const updateFormData = (section: 'job' | 'status' | 'notes' | 'tags', key: string | ApplicationStatus | string[], value?: any) => {
    if (section === 'status') {
  ...
  const addRequirement = () => {
    if (newRequirement.trim() && !formData.job.requirements.includes(newRequirement.trim())) {
      updateFormData('job', 'requirements', [...formData.job.requirements, newRequirement.trim()]);
      setNewRequirement('');
    }
  };

  const removeRequirement = (req: string) => {
    updateFormData('job', 'requirements', formData.job.requirements.filter(r => r !== req));
  };

  const addBenefit = () => {
    if (newBenefit.trim() && !formData.job.benefits.includes(newBenefit.trim())) {
      updateFormData('job', 'benefits', [...formData.job.benefits, newBenefit.trim()]);
      setNewBenefit('');
    }
  };

  const removeBenefit = (benefit: string) => {
    updateFormData('job', 'benefits', formData.job.benefits.filter(b => b !== benefit));
  };
  ...
  return (
    <Modal
      ...
      description={mode === 'create' ? 'Add a new job application to track' : 'Update application details'}
      size="xl"
    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateFormData('job', 'company', e.target.value)}
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateFormData('job', 'location', e.target.value)}
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateFormData('job', 'industry', e.target.value)}
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateFormData('job', 'salary', e.target.value)}
              />
            </div>
            <div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea
                ...
                onChange={(e) => updateFormData('job', 'description', e.target.value)}
              />
            </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {workArrangementOptions.map((option) => (
                    <button
                      key={option.value}
                      onClick={() => updateFormData('job', 'workArrangement', option.value)}
                      className={cn(
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {companySizeOptions.map((option) => (
                    <button
                      key={option.value}
                      onClick={() => updateFormData('job', 'companySize', option.value)}
                      className={cn(
                        "px-3 py-1 rounded-full text-sm transition-colors",
