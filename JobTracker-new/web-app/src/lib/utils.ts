...
import type { ApplicationStatus, JobApplication, UserProfile, Skill } from '@/types';
import { APPLICATION_STATUSES, DATE_FORMATS, VALIDATION_PATTERNS } from './constants';
...
export const dateUtils = {
  format: (date: Date | string, pattern: string = DATE_FORMATS.short): string => {
...
export const validationUtils = {
  ...
  isLinkedInProfile: (url: string): boolean => VALIDATION_PATTERNS.linkedIn.test(url),
  
  isGitHubProfile: (url: string): boolean => VALIDATION_PATTERNS.github.test(url),
...
export const jobUtils = {
  ...
  calculateProfileCompleteness: (profile: UserProfile): number => {
    ...
    if (profile.experience && profile.experience.length > 0) {
      score += 15;
      if (profile.experience.some(exp => exp.description && exp.description.length > 50)) {
        score += 15;
      }
    }
    ...
    if (profile.education && profile.education.length > 0) {
      score += 10;
      if (profile.education.some(edu => edu.description && edu.description.length > 20)) {
        score += 10;
      }
    }
  ...
  searchApplications: (applications: JobApplication[], query: string): JobApplication[] => {
  ...
  sortApplications: (applications: JobApplication[], sortBy: 'date' | 'status' | 'company' | 'title'): JobApplication[] => {
...
export const analyticsUtils = {
  calculateResponseRate: (applications: JobApplication[]): number => {
    const totalApplied = applications.filter(app => app.status !== 'draft').length;
    if (totalApplied === 0) return 0;
    
    const responded = applications.filter(app => 
      ['under_review', 'phone_screen', 'interview', 'final_interview', 'offer', 'rejected'].includes(app.status)
    ).length;
  ...
  calculateInterviewRate: (applications: JobApplication[]): number => {
    ...
    const interviewed = applications.filter(app => 
      ['phone_screen', 'interview', 'final_interview', 'offer'].includes(app.status)
    ).length;
  ...
  getTopCompanies: (applications: JobApplication[], limit: number = 5): string[] => {
...
export const getSkillRecommendations = (currentSkills: string[], targetRole: string): string[] => {
  // Simple skill recommendations based on role
  const roleSkillMap: Record<string, string[]> = {
    'frontend': ['React', 'TypeScript', 'CSS', 'HTML', 'JavaScript'],
    'backend': ['Node.js', 'Python', 'SQL', 'API Design', 'Docker'],
